...
import { HelpCircle, Search, Mail, ArrowLeft, ChevronDown, ChevronUp, AlertTriangle } from "lucide-react";
...
export default function HelpSwitcher() {
      ...
      <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#030816] via-[#09152a] to-[#020510] text-slate-200 p-5 pb-[max(2rem,env(safe-area-inset-bottom))]">
        ...
        <div className="shrink-0 flex items-center justify-between pt-[calc(env(safe-area-inset-top)+1rem)] pb-3 border-b border-white/[0.05]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-bold tracking-widest uppercase text-cyan-400">Support Console</span>
          </div>
        </div>

        {/* Title & Info */}
        <div className="py-4 shrink-0">
          <h1 className="text-lg font-extrabold tracking-tight text-white">Help Centre</h1>
          <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">
            Search user guides, anchor alarms, and safety procedures. For legal limitations, see our{" "}
        ...
        <div className="mb-4 shrink-0 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-white/[0.02] border border-white/[0.08] rounded-xl text-xs placeholder-slate-500 focus:outline-none focus:border-cyan-400 focus:bg-white/[0.04] transition-all"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          ...
          <aside className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 flex gap-3 shadow-inner">
            ...
            <div>
              ...
              <div className="mt-1.5 text-[10px] leading-relaxed text-slate-300 space-y-1.5">
          ...
          <div className="space-y-2.5">
            {filteredToc.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="border border-white/[0.05] bg-white/[0.01] rounded-2xl overflow-hidden transition-all duration-200"
                >
                  <button
                    onClick={() => toggleSection(item.id)}
                    className="w-full px-4 py-3 flex items-center justify-between text-left hover:bg-white/[0.03] active:bg-white/[0.04] transition-colors"
                  >
                  ...
                  {expanded && (
                    <div className="px-4 pb-4 pt-1 text-[11px] leading-relaxed text-slate-300 space-y-3 border-t border-white/[0.02] bg-black/10">
            ...
            {filteredToc.length === 0 && (
              <p className="text-center py-8 text-xs text-slate-500">No help topics matches your search.</p>
            )}
          </div>

          {/* Contact Developer Card */}
          <div className="bg-white/[0.02] border border-white/[0.05] rounded-2xl p-5 mt-6 space-y-4">
            <div className="flex items-center gap-2">
              <Mail size={16} className="text-cyan-400" />
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">Contact Developers</h3>
            </div>
            <p className="text-[10.5px] text-slate-400 leading-normal">
              Bug reports, feature ideas, and account questions can go straight to the SeaLink developers. Email template pre-fills device parameters automatically.
            </p>
            <a
              href={mailtoDevelopers}
              className="w-full inline-flex h-10 items-center justify-center rounded-xl bg-cyan-600 hover:bg-cyan-500 active:scale-[0.98] text-xs font-semibold text-white shadow-lg shadow-cyan-950/40 transition-all"
            >
              Email developers
            </a>
            <p className="text-[9.5px] text-slate-500 text-center">
              If the button does nothing, copy address manually: <span className="font-mono text-slate-400">{DEV_EMAIL}</span>
            </p>
          </div>
  ...
  // --- DESKTOP LAYOUT (Default Unchanged) ---
  return (
...
function renderSectionBody(id: string) {
  switch (id) {
    ...
    case "navigation-charts-colregs":
      return (
        <>
          <p>
            <Link href="/navigation-charts" className="font-medium text-emerald-400 hover:underline">
    ...
    case "broadcasts":
      return (
        <>
          ...
          <ul className="list-disc space-y-1 pl-5 text-zinc-400">
            <li>Broadcasts are not emergency services — use VHF/DSC and phone to 999/112/911 for distress.</li>
    ...
    case "messages-broadcast-audience":
      return (
        <>
          <p>
          ...
          <p>
            ...
            they are on your list and using IFM with a compatible identity.
          </p>
    ...
    case "anchor-email-second-device":
      return (
        <>
          <p className="text-zinc-400">

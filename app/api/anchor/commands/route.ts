...
export async function GET(req: Request): Promise<Response> {
  ...
  if (url.searchParams.get("role") === "monitor") {
    try {
      ...
      console.error("[api/anchor/commands GET role=monitor]", msg);
      const hint =
        /anchor_session_commands|column|relation|PGRST/i.test(msg)
          ? "Apply Supabase migration supabase/migrations/024_anchor_session_commands.sql on production."
          : undefined;
      return NextResponse.json(
        {
          commands: [],

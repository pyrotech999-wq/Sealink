...
export const metadata: Metadata = {
  title: "Delete your data | SeaLink",
...
function liveDeleteDataUrl(): string {
  const o = resolvePublicAppOrigin();
  return `${o.replace(/\/$/, "")}/delete-data`;
}

export default function DeleteDataPage() {
  const canonicalUrl = liveDeleteDataUrl();
  ...
  return (
    <div className="flex flex-1 flex-col bg-black">
      ...
      <main className="mx-auto w-full max-w-lg flex-1 px-4 py-10 sm:px-6 sm:py-14">
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">Delete your data</h1>
        <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
          Canonical URL:{" "}
          <a href={canonicalUrl} className="font-mono text-emerald-600 hover:underline dark:text-emerald-400">
            {canonicalUrl}
          </a>
        </p>
        <p className="mt-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
          This page is open to everyone — you do <strong className="text-zinc-800 dark:text-zinc-200">not</strong> need to
          be signed in to read the steps below. Deleting your account and associated data cannot be undone once you
          confirm.
        </p>

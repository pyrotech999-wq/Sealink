...
import HomeSwitcher from '@/components/mobile/home/HomeSwitcher';
...
export default async function Home() {
  ...
  const welcomeFirstName = authUser
    ? await getProfileFirstNameForUser(authUser.uid)
    : null;
        ...
        <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col px-4 py-8 sm:px-6 sm:py-10">
          ...
          {!signedIn ? (
            <p className="mt-3 max-w-2xl text-base leading-7 text-zinc-400">
              Your home screen includes a live map: share your GPS position,
              show your boat name and profile photo on the pin, and keep slower
              background updates on by default while the tab stays open (you can
              pause on the map). Fully closed browsers cannot keep GPS on a
              normal website — that needs a native wrapper app.
            </p>
          ) : null}
          {signedIn ? (
            <p className="mt-4 text-sm text-green-300">
              You&apos;re signed in. Posting adverts and broadcasts is tied to
              your account email.
            </p>
          ) : null}
